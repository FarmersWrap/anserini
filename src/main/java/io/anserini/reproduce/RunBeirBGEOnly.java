...
/**
 * Runner for BEIR BGE-only experiments (dense search with ONNX encoder).
 * 
 * Output files: runs/run.beir-bge-only.bge-base-en-v1.5.flat.onnx.{corpus}.txt
 */
public class RunBeirBGEOnly {
  ...
  public static void main(String[] args) throws Exception {
    ...
    RunRepro repro = new RunRepro("beir-bge-only", new BeirBGEOnlyMetricDefinitions(), 
        beirArgs.printCommands, beirArgs.dryRun, beirArgs.computeIndexSize);
    repro.run();
  }

  public static class BeirBGEOnlyMetricDefinitions extends TrecEvalMetricDefinitions {
    public BeirBGEOnlyMetricDefinitions() {
      super();
      ...
      metricDefinitions.put("beir-bge-only", beir);
    }
  }
}
